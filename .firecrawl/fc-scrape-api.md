...
If a request fails, see [Errors](https://docs.firecrawl.dev/api-reference/errors) for the full catalog of error codes, causes, remedies, and retry guidance.
...
**PDFs and documents:**`/scrape` auto-detects PDFs, DOCX, and other document types from URLs. Pass a PDF URL the same way you would any webpage — Firecrawl parses it and returns clean markdown. For local files that are not accessible by URL, use [`/parse`](https://docs.firecrawl.dev/features/parse) instead.
...
\
Extracting products? For product pages, the [`product` format](https://docs.firecrawl.dev/features/scrape#extract-product-data) returns structured product fields (title, price, availability, variants) deterministically — no LLM call and no schema to define. Reach for `json` when you need custom fields or non-product pages.\
\
Python\
\
Node\
...
```\
\
### [​](https://docs.firecrawl.dev/features/scrape\#product-object-structure)  Product object structure\
\
...
\
### [​](https://docs.firecrawl.dev/features/scrape\#how-product-extraction-works)  How product extraction works\
\
...
\
### [​](https://docs.firecrawl.dev/features/scrape\#combining-with-other-formats-2)  Combining with other formats\
\
...
```\
\
## [​](https://docs.firecrawl.dev/features/scrape\#video-extraction)  Video extraction\
\
...
```\
\
## [​](https://docs.firecrawl.dev/features/scrape\#question-format)  Question format\
\
...
\
## [​](https://docs.firecrawl.dev/features/scrape\#highlights-format)  Highlights format\
\
...
\
## [​](https://docs.firecrawl.dev/features/scrape\#pii-redaction)  PII redaction\
\
Set `redactPII: true` to redact personally identifiable information from returned markdown. The `markdown` field contains the redacted result.See [PII Redaction](https://docs.firecrawl.dev/features/pii-redaction) for SDK, cURL, CLI, and MCP examples.\
\
...
\
**We recommend [Interact](https://docs.firecrawl.dev/features/interact) over actions: our newer, more powerful way to interact with scraped pages.**Interact runs as a stateful browser session that stays alive across calls, so you can drive a page turn-by-turn with either:\
\
- **Natural language** for flexible, non-deterministic flows. e.g. _“search for ‘wireless headphones’, filter to 4+ stars under $200, and return the results”_.\
...
```\
\
For workflows that require richer browser control after scraping, such as authenticated sessions, multi-step navigation, or a live view of the page, we recommend [Interact](https://docs.firecrawl.dev/features/interact) over extending the actions array.\
\
...
```\
\
### [​](https://docs.firecrawl.dev/features/scrape\#response-4)  Response\
\
